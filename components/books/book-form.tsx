...
export function BookForm({ initialValue, bookId }: { initialValue?: BookInput; bookId?: string }) {
	...
	const loanManagedStatus = Boolean(bookId && initialValue?.status === 'BORROWED');
	...
	return (
		<div className="book-form-flow">
			...
			>
				...
				{!selected && !bookId ? (
					<div className="book-selection-placeholder">
						<SearchIcon />
						<strong>먼저 위에서 책을 검색해 선택해 주세요</strong>
						<p>선택하면 표지와 기본 정보가 자동으로 채워져요.</p>
					</div>
				) : (
					<div className="selected-book-layout">
						<aside>
							<BookCover
								...
								title={values.title || '책'}
								sizes="190px"
								priority
							/>
							<p>
								<CheckCircle2 size={16} /> 카카오 도서 정보
							</p>
						</aside>
						<div className="book-form-fields">
							...
							<Field
								label="보관 장소 또는 전달 방법"
								error={errors.location?.message}
							>
								<input
									placeholder="예: 판교 오피스 8층, 사내 우편 가능"
									{...register('location')}
								/>
							</Field>
							<Field
								...
								{loanManagedStatus ? (
									<>
										<input
											...
											{...register('status')}
										/>
										<div className="book-status-locked">
											<CheckCircle2 size={17} />
											대여 중 · 책 상세에서 대여자를 변경할 수 있어요
										</div>
									</>
								) : (
									<select {...register('status')}>
										<option value="AVAILABLE">대여 가능</option>
										{!bookId && <option value="BORROWED">대여 중</option>}
										<option value="UNAVAILABLE">대여 불가</option>
									</select>
								)}
							</Field>
							{values.status === 'BORROWED' && !bookId && (
								<div className="book-form-field">
									<MemberPicker
										name={values.currentBorrowerName ?? ''}
										memberId={values.currentBorrowerId ?? ''}
										onChange={(name, id) => {
											setValue('currentBorrowerName', name, {
												shouldValidate: true,
											});
											setValue('currentBorrowerId', id, {
												shouldValidate: true,
											});
										}}
									/>
									{errors.currentBorrowerName?.message && <small>{errors.currentBorrowerName.message}</small>}
								</div>
							)}
							<input
								type="hidden"
								{...register('currentBorrowerId')}
							/>
							<input
								type="hidden"
								{...register('currentBorrowerName')}
							/>
							<input
								type="hidden"
...
function SearchIcon() {
	return (
		<span
			aria-hidden="true"
			className="selection-search-icon"
		>
			⌕
		</span>
	);
