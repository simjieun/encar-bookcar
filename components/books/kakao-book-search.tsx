...
export function KakaoBookSearch({ onSelect }: { onSelect: (book: KakaoBook) => void }) {
	...
	return (
		<section
			className="kakao-search-panel"
			aria-labelledby="kakao-search-heading"
		>
			...
			>
				...
				<button
					type="submit"
					disabled={input.trim().length < 2 || result.isFetching}
				>
					검색
				</button>
			</form>
			...
			{result.isError && (
				<div
					className="kakao-search-error"
					role="alert"
				>
					{result.error.message}
					<small>관리자에게 카카오 책 검색 설정을 확인해 달라고 알려주세요.</small>
				</div>
			)}
			{result.data && result.data.items.length === 0 && (
				<div className="kakao-search-message">
					<SearchX size={22} />
			...
			{result.data && result.data.items.length > 0 && (
				<div
					className="kakao-result-list"
					aria-label="카카오 책 검색 결과"
				>
					{result.data.items.map((book, index) => (
						...
						>
							...
							<span>
								...
								<small>{book.author || '저자 정보 없음'}</small>
