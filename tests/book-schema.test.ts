...
describe("도서 입력 검증", () => {
  ...
  it("잘못된 상태와 출간일을 거부한다", () => {
    expect(bookSchema.safeParse({ ...validBook, status: "LOST", publishedAt: "2026-08-11" }).success).toBe(false);
  });
  ...
  it("목록 검색 기본값을 안전하게 적용한다", () => {
    expect(bookListQuerySchema.parse({})).toEqual({ query: "", status: "ALL", sort: "latest", page: 1 });
  });
});
