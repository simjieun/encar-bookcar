...
function renderDiscovery() {
  ...
  return render(<QueryClientProvider client={queryClient}><BookDiscovery /></QueryClientProvider>);
}
...
beforeEach(() => {
  vi.stubGlobal("fetch", vi.fn(async (input: string | URL | Request) => {
    ...
    return { ok: true, json: async () => ({ books: empty ? [] : [sampleBook], hasMore: false, page: 1 }) } as Response;
  }));
});
...
describe("BookDiscovery", () => {
  it("실제 도서 API에서 책을 표시하고 검색한다", async () => {
    ...
    const search = screen.getByRole("searchbox", { name: "책 제목 또는 저자 검색" });
